...
import { getWithdrawalRecords } from "./api"; // same pattern as getDepositRecords
...
export default function WithdrawalRecords() {
  ...
  const fetchRows = useCallback(
    async (showSpinner = false) => {
      try {
        ...
        setRefreshing(true);
        const res = await getWithdrawalRecords(String(userId));
  ...
  // auto-refresh on focus + light polling (like deposit page)
  useEffect(() => {
  ...
  return (
    <div className="page" style={{ background: "#f6f7fb", minHeight: "100%" }}>
      {/* === SAME TOPBAR + BACK BUTTON AS DEPOSIT PAGES === */}
      <header
