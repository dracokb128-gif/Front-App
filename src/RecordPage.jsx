...
import { snapshotFromTask, pushCompleted, listCompleted, hydrateFromDisplay } from "./utils/history";
...
function CenterTip({ show, text, anchor }) {
  ...
  const wrap = anchored
    ? { position: "fixed", left: `${anchor.cx}px`, top: `${anchor.top}px`, transform: "translate(-50%,0)", zIndex: 300005, pointerEvents: "none" }
    : { position: "fixed", left: 0, right: 0, bottom: 360, display: "flex", justifyContent: "center", zIndex: 300005, pointerEvents: "none" };
  return createPortal(
    <div style={wrap}>
      <div
        style={{
...
export default function RecordPage() {
  ...
  async function handleSubmit() {
    ...
    if (latestNeed > 0.0005) {
      setTip({ show: true, text: `Your account balance is not enough, you need to recharge ${latestNeed.toFixed(3)} USDT to submit this order` });
      return;
    }
    ...
    <div className="wrap record-page" style={{ paddingBottom: 90 }}>
      ...
      {tab === "incomplete" && task ? (
        <div ref={cardRef} className="card" style={{ background: "#fff", borderRadius: 14, boxShadow: "0 10px 24px rgba(0,0,0,.06)", padding: 16 }}>
          ...
          {/* TODO: render task.items list just like before */}
        </div>
      ) : (
