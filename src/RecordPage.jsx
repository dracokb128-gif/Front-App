...
/* ---------- deterministic synth (no localStorage; stable across re-login) ---------- */
function hashStr(s = "") {
  let h = 2166136261 >>> 0;
...
/** Pick N distinct products deterministically from PRODUCTS using orderId-based seed */
...
/** Build line items (title/image from stable picks; qty/price seeded as well) */
function synthesizeItems(task) {
  ...
  // Try to make amounts “nice”, but if orderAmount exists, UI will use that.
  let remaining = Math.max(8000, Math.round((toNum(task?.orderAmount) || 9555) * 100)); // cents
  const items = [];

  for (let i = 0; i < picks.length; i++) {
    const qty = 1 + Math.floor(rnd() * 3);
    let cents = 500 + Math.floor(rnd() * 3000); // 5.00 – 35.00
...
export default function RecordPage() {
  ...
  async function handleSubmit() {
    ...
    try {
      if (rawUnpaid) {
        try { await submitUnpaid(userId, rawUnpaid); } catch {}
      }
      ...
      const data = err?.data || {};
      const need = Number.isFinite(Number(data.deficit)) ? Number(data.deficit) : Math.max(0, Number(cashGap || task.deficit || 0));
            ...
            <div style={{ border: "1px solid #eef2f7", borderRadius: 10, padding: 10, marginTop: 6 }}>
              {task.kind === "combine" ? (
                  ...
                  {task.items.slice(0, 5).map((it, idx) => {
                    ...
                    return (
                      ...
                      >
                        ...
                        <div style={{ minWidth: 0 }}>
                          <div style={{ fontWeight: 600, fontSize: 14, whiteSpace: "nowrap", overflow: "hidden", textOverflow: "ellipsis" }} title={title}>
                            {title}
                          </div>
                        ...
                        <div style={{ color: "#94a3b8", fontSize: 13, fontWeight: 600, paddingLeft: 6 }}>x{Number(it.quantity || 1)}</div>
                      </div>
                    );
                  })}
                </div>
                ...
                <div style={{ display: "grid", gridTemplateColumns: "64px 1fr auto", gap: 10, alignItems: "center" }}>
                  ...
                  <div style={{ minWidth: 0 }}>
                    <div style={{ fontWeight: 600, fontSize: 14, whiteSpace: "nowrap", overflow: "hidden", textOverflow: "ellipsis" }}>{task.title || "Order"}</div>
